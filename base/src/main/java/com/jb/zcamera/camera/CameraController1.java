...
public class CameraController1 extends CameraController {
	...
	public class CameraHolder1 implements CameraHolder {

		private Camera camera;

		public CameraHolder1(Camera camera) {
			this.camera = camera;
		}
	...
	public CameraController1(int cameraId, ErrorCallback callback) {
		camera = Camera.open(cameraId);
		cameraHolder = new CameraHolder1(camera);
	...
	public synchronized CameraHolder getCamera() {
		return cameraHolder;
	}
		...
		if(mErrorCallback != null) {
			camera.setErrorCallback(new Camera.ErrorCallback() {
				...
				public void onError(int i, Camera camera) {
					mErrorCallback.onError(i, new CameraHolder1(camera));
				}
			});
		}
		camera.startPreview();
    ...
    synchronized void setFaceDetectionListener(final FaceDetectionListener listener) {
	...
	synchronized void autoFocus(final AutoFocusCallback cb) {
	...
	synchronized void setAutoFocusMoveCallback(final AutoFocusMoveCallback cb) {
	...
	synchronized void takePicture(final PictureCallback raw, final PictureCallback jpeg, boolean shutterSound) {
